       ...
       // echo "insert into facturacion (fecha, id_cliente, id_forma_pago, fecha_vencimiento,cerrado,total) values(now(), $_POST['clientes'], $_POST['condicion_venta'], '$_POST[fecha_vencimiento]', 0, '$_POST[total]') RETURNING *;";
        ...
        if(!mysqli_error($con))
        {
            ...
            if(!mysqli_error($con))
            {
                ...
                echo "<script>window.open('presupuesto_pdf.php?id=".$r[id]."');window.location='home.php?pagina=compras';</script>";
               ...
               <div id="accordion">
                     <!-- Page Heading -->
                        <div class="card shadow mb-4" id="headingOne">
                            ...
                            <h6 class="m-0 font-weight-bold text-primary" data-toggle="collapse show" data-target="#collapseNuevo" aria-expanded="true" aria-controls="collapseNuevo">Nueva Compra</h6>
                            </div>
               ...
               <?php
                        ...
                        if($_GET[ver]!=0)
                        {
                            $sql=mysqli_query($con,"select *from compras where id_compra=$_GET[ver]");
                                ...
                                $url="home.php?pagina=compras&mod=ok";
                                $showform="show";
                        }
                            else
                            {
                                $url="home.php?pagina=compras&add=ok";
                                $showtable="show";
                            }
                    ?>
