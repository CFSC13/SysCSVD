 ...
 else
 {
  $sql=mysqli_query($con, "select i.*, gm.icono from syscsvd_items_interno i, syscsvd_items_x_usuario iu, syscsvd_usuarios u, syscsvd_grupos_menu gm where i.id_grupo=gm.id_grupo and iu.id_item=i.id_item and iu.id_usuario=u.id_usuario and u.id_usuario=".$_SESSION['user']." order by gm.nombre_grupo, i.nombre_item");
		...
		while($r=mysqli_fetch_array($sql))
		{
			if($r['id_grupo']!=$grupo)
			{
				...
				<li class="nav-item">
		          <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapseTwo<?php echo $r['id_grupo'];?>"
		              ...
		              <span><?php echo ucfirst(datos_complejos1($con, "syscsvd_grupos_menu","nombre_grupo","id_grupo",$r['id_grupo']));?></span>
		          </a>
			...
			$ur=explode(".",$r['url']);
			?>
			<a class="collapse-item" href="home.php?pagina=<?php echo $ur[0]; ?>"><?php echo ucfirst(datos_complejos1($con, "syscsvd_items_interno","nombre_item","id_item",$r['id_item']));?></a>
			<?php
		}
		?>
		</div>
