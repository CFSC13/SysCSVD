...
if($_GET['add']=="ok")
{
    if($_POST['cod_prod']!="" && $_POST['can_prod']!="")
    {
       // echo "insert into facturacion (fecha, id_cliente, id_forma_pago, fecha_vencimiento,cerrado,total) values(now(), $_POST['clientes'], $_POST['condicion_venta'], '$_POST[fecha_vencimiento]', 0, '$_POST[total]') RETURNING *;";
        $sql=mysqli_query($con,"insert into syscsvd_ventas (fecha_de_venta, importe_total, id_usuario) values(now(), '$_POST[total]',  $_POST[id_usuario])");
        //echo "'$_POST[total]', '$_POST[cod_prod]', now(), '$_POST[id_usuario]'";
        

        if(!mysqli_error($con))
        {
            $r=mysqli_fetch_array(mysqli_query($con,"select MAX(id_venta) as id from syscsvd_ventas"));
            ...
            while($n<=$cant_articulos){
                if($_POST['cod_prod'][$n]!="" && $_POST['can_prod'][$n]){
                    ...
                    $rp=mysqli_fetch_array(mysqli_query($con,"select precio from syscsvd_productos where id_producto='$cod'"));
                    //echo "$rp";
                    $subtotal=$rp['precio']*$can;
                    $sql2.="insert into syscsvd_detalle_ventas (id_producto,id_venta,precio_unitario,cantidad,subtotal) values('".$cod."', $r[id], $rp[precio], $can, '$subtotal');";
            ...
            if(!mysqli_error($con))
            {
                ...
                echo "<script>window.location='home.php?pagina=ventas_scanner';</script>";
  ...
  <div class="tab-content" id="nav-tabContent">                         
            <div class="tab-pane fade show active" id="nav-home" role="tabpanel" aria-labelledby="nav-home-tab">
               <div id="accordion">
                     <!-- Page Heading -->
                        <div class="card shadow mb-4" id="headingOne">
                            ...
                            <h6 class="m-0 font-weight-bold text-primary" data-toggle="collapse show" data-target="#collapseNuevo" aria-expanded="true" aria-controls="collapseNuevo">Escanea el Producto</h6>
                            </div>
               ...
               <?php
                        ...
                        if($_GET[ver]!=0)
                        {
                            $sql=mysqli_query($con,"select *from syscsvd_ventas where id_venta=$_GET[ver]");
                                if(mysqli_num_rows($sql)!=0)
                                {   
                                    $r=mysqli_fetch_array($sql);
                                }
                                $url="home.php?pagina=ventas_scanner&mod=ok";
                                $showform="show";
                        }
                            else
                            {
                                $url="home.php?pagina=ventas_scanner&add=ok";
                                $showtable="show";
                            }
                    ?>
                        ...
                        <fieldset class="border p-2">
                            ...
                            <div style="width: 270px" id="reader"></div>
                               ...
                               <fieldset class="border p-2">
                                ...
                                <p style="text-align: center;"><button type="button" onclick="AddProductos()" class="btn btn-primary">Agregar</button></p>
                                <br>
...
<script type="text/javascript">
    ...
    function updateSubtotal(n, precio) {
            ...
            let subtotal = precio * can;
            $("#subtotal_" + n).html("$" + new Intl.NumberFormat().format(subtotal.toFixed(2)));
            $("#can_prod_" + n).val(can);
            let total = 0;
            $(".input-qty").each(function() {
                let subtotal = parseFloat($(this).val()) * parseFloat($("#precio").val());
                total += subtotal;
            });
            $("#totalMostrado").html("Total: $" + new Intl.NumberFormat().format(total.toFixed(2)));
            $("#total").val(total);
         }
